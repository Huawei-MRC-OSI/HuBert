...
GITHACK=n
MAPSOCKETS=y
DOCKERFILE=$CWD/docker/HuBert.docker

while test -n "$1" ; do
  case "$1" in
    -h|--help)
      echo "Usage: $0 [--dev] [-n|--no-map-sockets]" >&2
      exit 1
      ;;
    -n|--no-map-sockets)
      MAPSOCKETS=n
...
DOCKER_CFG="/tmp/docker-HuBert-$UID"
...
DOCKER_IMGNAME="$USER-hubert-`basename $DOCKERFILE .docker | tr '[:upper:]' '[:lower:]'`"
